    ...
    for index, row in data.iterrows():
        ...
        # also prints which row pass and fail                                
            ...
            if check == 0 or check == 3:
                ...
                log = log + f'{time.strftime("%I:%M:%S %p")}: row {index} fail on ID check, pleas check that ID is present, is int, or not duplicated\n'

            elif check == 1 or check == 4:

                log = log + f'{time.strftime("%I:%M:%S %p")}: row {index} fail on name check, pleas check that name is present\n'

            elif check == 2 or ckeck == 6: 

                log = log + f'{time.strftime("%I:%M:%S %p")}: row {index} fail on email check, pleas check that email is present and is in proper format\n'

            elif check == 5:

                log = log + f'{time.strftime("%I:%M:%S %p")}: row {index} fail on sales check, pleas check that sales is int\n'

            else:

                log = log + f'{time.strftime("%I:%M:%S %p")}: row {index} fail on department check, pleas check that feild contains vaild department\n'
