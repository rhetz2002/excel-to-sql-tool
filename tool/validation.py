...
def validate(filepath, data):
    ...
    for index, row in data.iterrows():
        ...
        if pd.notna(row['ID']):
            ...
            if (
                pd.notna(row['name'])
                and not any(char.isdigit() for char in str(row['name']))
                and len(str(row['name'])) >= 3
                and ' ' in str(row['name'])
            ):

                # checks if email column is not empty,
                # contains an @ symbol as well as .com,
                ...
                if (
                    ...
                    and '.com' in str(row['email'])
                    and len(str(row['email'])) >= 9
                ):
                    ...
                    try:
                        ...
                        # if it is not a float sets
                        # check to six to set the error
                    ...
                    if (
                        ...
                        or str(row['department']).strip().lower()
                        ...
                        try:
                            ...
                            if int(row['ID']) not in id_check:
                                ...
                                # adds one to success to
                                # keep track of valid rows
                            ...
                            # if it is not an integer sets
                            # check to four to set the error

                            check = 4

                    else:

                        # if it is not valid sets
                        # check to eight to set the error
