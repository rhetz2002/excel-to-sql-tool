 | Defect ID | Description | Steps to Reproduce | Severity | Status | Fix Commit |
  |-----------|-------------|--------------------|----------|--------|------------|
  | D-001 | Tool crashes if a reprompt fails to open a present file | point tool to an invalid input/output input an invalid input/output on the re-prompt | High |Open|...|
